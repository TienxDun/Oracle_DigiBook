...
> **Cập nhật theo trạng thái database hiện tại:** ngoài 15 bảng nghiệp vụ lõi ở trên, hệ thống hiện có thêm bảng vận hành `ORDERS_AUDIT_LOG` (được tạo ở bước Trigger) để lưu audit thao tác trên `ORDERS`.
...
Các index đang được triển khai trong bộ script hiện tại:
- `UQ_BIMG_PRIMARY_ONE` (unique function-based index) trên `BOOK_IMAGES`, đảm bảo mỗi sách chỉ có tối đa một ảnh chính.
- `IDX_ORDERS_RECENT_DATE` trên `ORDERS(order_date DESC, order_id DESC)` để tối ưu dashboard đơn hàng gần nhất.
- `IDX_BOOKS_LOW_STOCK` trên `BOOKS(stock_quantity, book_id)` cho nghiệp vụ cảnh báo tồn kho thấp.
- `IDX_ORDERS_TRUNC_ORDER_DATE` (function-based index) trên `TRUNC(ORDERS.order_date)` cho báo cáo theo ngày.
- `IDX_BOOKS_CATEGORY_BM` (bitmap index) trên `BOOKS(category_id)` cho truy vấn đọc nhiều theo danh mục.

Ngoài ra, các ràng buộc `PRIMARY KEY` và `UNIQUE` cũng tạo các index ngầm định đi kèm.

### 4.15. Kiểm soát vòng lặp danh mục (circular reference) — đề xuất mở rộng

`CATEGORIES` là cấu trúc tự tham chiếu nên nên có trigger `BEFORE INSERT/UPDATE` để chặn:
- `parent_id = category_id` (self-parent).
- Trường hợp tạo vòng lặp cha-con khi duyệt ngược cây danh mục.

> **Trạng thái hiện tại:** trigger chống vòng lặp danh mục chưa được triển khai trong các script hiện có.
...
| Tổng số bảng hiện tại trong schema | **16** (15 bảng lõi + `ORDERS_AUDIT_LOG`) |
...
| Sequences | **15** |
| Triggers | **17** |
| Index tùy biến khai báo tường minh | **5** |
...
### 9.8. Thiếu logging/monitoring cho API errors ⚠️
...
| **Thời hạn** | **Khuyến khích** triển khai sớm (nếu chưa); **bắt buộc** trước khi có feature order delivery. |
...
2. **Giai đoạn Staging (Trước Production):**
    - Implement stored procedure xử lý race condition của coupon bằng cơ chế lock phù hợp.
   - Setup logging/monitoring.
   - Input validation trên tất cả endpoints.
   - Backup/DR plan & testing.
    - Duy trì trạng thái đã loại bỏ cột legacy `cover_image_url` trong mọi luồng mới.
